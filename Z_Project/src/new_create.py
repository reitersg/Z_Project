...
# create2.py
# Added code for wait for angle, wait for distance,
#   wait for event (C.A. Berry) - 8/22/08
# Added code for sensor streaming, removed obsolete,
#   unnecessary code (integrate odometery) (CAB) - 8/26/08
# create3.py
# Added the senseAndRetry code from prior versions
# Modified the moveTo(0 adn turnTo() functions
#   to work with go instead of drive function
# NOTE: renamed back to create.py - 9/16/08
# v1.1 Added access to overcurrent; made sensor streaming optional
#  in the Create constructor. This allows
...
# v2.0 Complete overhaul, removing many duplicate functions
#   not used by students.
# Removed threaded sensor polling: less efficient if many sensors needed,
...
# v2.2 Added support for a simulator to connect via a network socket
#  (RHIT Senior Project Team 9/12/09)
# v2.3 Added ability to retry getting sensor data.
# v3.0 (FIXME: rename shutdown as disconnect)
...
# FIXME: define the rest of the command codes in the SCI.
...
class Create:
    ...
    # FIXME: check if we can start in other modes...
#======================== Starting up and Shutting Down================
    def __init__(self, PORT, startingMode=FULL_MODE, sim_mode=False):
        ...
        # otherwise, we try to open the numeric serial port...
            ...
            try:
                ...
                print("")
                print("I am unable to make a physical connection to the robot.")
                print("There can be many possible reasons, including:")
                print("  -- Is the robot turned on?")
                print("  -- Is the COM port number correct?")
                print("  -- Is the robot's battery charge adequate?")
                print("Most likely, however, you failed to execute a  shutdown")
                print("on the previous run and left the robot in a funky state.")
                print("To fix this, try turning the robot off, then on again.")
                print("Or, simply switch to another robot (but keep the same BAM).")
#                 raise Exception('Could not connect to the robot')
        ...
        self.print_all_interesting_sensors()
    ...
    def shutdown(self):
        ...
        '''
        self.stop()
        self.setLEDs(100, 255, 0, 0)  # DCM: I think the following code changes the lights anyhow, not sure.
...
#======================= CARGO BAY OUTPUTS ==========================
    ...
    def startIR(self, byteValue):
        '''FIXME: implement script send to begin sending passed value'''
